...
// *Request 指针必须以内存地址返回，以免消耗资源
func NewRequest(url string, verify string, code int, timeout time.Duration, debug bool, path string,
	reqHeaders []string,
	reqBody string, maxCon int, http2 bool, keepalive bool,redirect bool) (request *Request, err error) {
	//赋参
	var (
		method  = "GET"
	...
	if path != "" {
		var curl *CURL
		curl, err = ParseTheFile(path) //如果-P参数不为空，则读取curl文件
		if err != nil {
			return nil, err
		}
		if url == "" {
		...
		//如果headers的content-type键为空。OK代表有没有这个键值对。_为忽略值
...
// RequestResults 请求结果 通过管道传输，统计结果 请求时+ 输出结果时-
